...
foreach ($size in @(@(1920,1080),@(1440,900),@(1280,720),@(390,844),@(430,932))) {
    ...
    foreach ($mode in @('light','dark')) {
        foreach ($skin in @('normal','spider','venom','espetacular','santos','flamengo','sao-paulo','bolsonaro')) {
...
Write-Output 'SKIN_LAYOUT=PASS (5 sizes x 2 base modes x 8 skins, contrast >= 4.5)'
...
foreach ($skin in @('spider','venom','espetacular','santos','flamengo','sao-paulo','bolsonaro','normal')) {
...
Invoke-JS 'window.__setTestSkin = value => {document.getElementById("visualTheme").value=value;document.getElementById("visualTheme").dispatchEvent(new Event("change"))};window.__setTestSkin("bolsonaro")' | Out-Null
if ((Invoke-JS 'getComputedStyle(document.body,"::after").animationName') -ne 'brazil-glow') { throw 'Brazil glow did not start.' }
Invoke-JS 'window.__setTestSkin("santos")' | Out-Null
if ((Invoke-JS 'getComputedStyle(document.body,"::after").animationName') -ne 'none') { throw 'Glow leaked into another theme.' }
Invoke-JS 'window.__setTestSkin("bolsonaro")' | Out-Null
...
foreach ($skin in @('spider','venom','espetacular','santos','flamengo','sao-paulo','bolsonaro')) {
    Invoke-JS ('document.getElementById("visualTheme").value="' + $skin + '";document.getElementById("visualTheme").dispatchEvent(new Event("change"))') | Out-Null
